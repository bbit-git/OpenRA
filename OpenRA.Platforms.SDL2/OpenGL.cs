...
namespace OpenRA.Platforms.SDL2
{
	...
	static class OpenGL
	{
		...
		public delegate void DebugProc(int source, int type, uint id, int severity, int length, IntPtr message,
			IntPtr userParam);

		// Use a static readonly function pointer for UnmanagedCallersOnly methods
		static unsafe delegate* unmanaged<int, int, uint, int, int, IntPtr, IntPtr, void> DebugMessageHandle { get; set; }

		public unsafe delegate void DebugMessageCallback(delegate* unmanaged<int, int, uint, int, int, IntPtr, IntPtr, void> callback, IntPtr userParam);
		...
		static void DebugMessageHandler(int source, int type, uint id, int severity, int length, IntPtr messagePtr, IntPtr userparam)
		{
			string error;
			var message = new StringBuilder(Marshal.PtrToStringAnsi(messagePtr, length));
